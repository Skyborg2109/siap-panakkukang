...
- Routes/pages: `public/` is only Home/Information/IKD; `display/Display.jsx`; `auth/Login.jsx` (own chrome — don't wrap in `pub()`); `petugas/` (Queue/History/Profile, `/petugas` → `/petugas/queue`); `admin/` via `AdminShell` + `DashboardLayout` in `src/layouts/layouts.jsx`.

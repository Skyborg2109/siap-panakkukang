...
- TTS `useSpeech()` (`hooks.js`): `id-ID`, toggle in `siap_tts`, volume slider in `siap_volume` (max 1.0 — browser cap, code can't amplify beyond system 100%), opening sound → announcement 2x → closing sound. Audio filenames contain a space (`opening sound.mp3`) — don't rename without updating `hooks.js`. Display TTS stays silent until first user gesture (browser autoplay policy).

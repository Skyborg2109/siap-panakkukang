...
- Petugas panel never uses `alert()` for call flows — errors/success go through the in-app `flash()` banner (`errText()` maps timeouts), and every mutation is wrapped in `withTimeout(20s)`. TV/kiosk browsers often block dialog popups, which otherwise looks like "nothing happens".

...
- Services can't be hard-deleted once they have queue rows (`queues.service_id on delete restrict` in `schema.sql`): `deleteService` falls back to `is_active=false` on FK error `23503` — don't "fix" by force-deleting, history/statistics depend on it.
